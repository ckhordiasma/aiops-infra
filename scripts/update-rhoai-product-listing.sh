#!/usr/bin/env bash
# update-rhoai-product-listing.sh — Adds component to RHOAI product listing via MR to pyxis-repo-configs
#
# Usage:
#   ./scripts/update-rhoai-product-listing.sh [<jira-url>] [--existing-mr-url <url>]
#   ./scripts/update-rhoai-product-listing.sh https://redhat.atlassian.net/browse/RHOAIENG-1234
#
# Required env vars:
...
[[ -z "$COMPONENT_NAME" ]] && {
  echo "ERROR: Missing required field 'component_name' in component_onboarding_details.yaml."
...
# ── Fast-path check: does entry already exist? ────────────────────────────────
...
if [[ "$HTTP_STATUS" != "200" ]]; then
  echo "WARN: Could not fetch product-listings/rhoai/rhoai.yaml via GitLab API (HTTP $HTTP_STATUS)."
  echo "  Ensure VPN is active. Continuing with clone."
  rm -f "$RHOAI_YAML_TMPFILE"
elif grep -qF "$PRODUCT_LISTING_ENTRY" "$RHOAI_YAML_TMPFILE"; then
  rm -f "$RHOAI_YAML_TMPFILE"
...
[[ -f "$RHOAI_YAML" ]] || {
  echo "ERROR: product-listings/rhoai/rhoai.yaml not found in $CLONE_DIR."
...
if grep -qF "$PRODUCT_LISTING_ENTRY" "$RHOAI_YAML"; then
  ...
  python3 "$SCRIPTS_DIR/append_yaml_list_entry.py" "$RHOAI_YAML" \
    ...
    echo "ERROR: Could not append entry to product-listings/rhoai/rhoai.yaml. See details above."
    exit 1
  }

  grep -qF "$PRODUCT_LISTING_ENTRY" "$RHOAI_YAML" || {
    echo "ERROR: Verification failed — '$PRODUCT_LISTING_ENTRY' not found after append."
    exit 1
  }
