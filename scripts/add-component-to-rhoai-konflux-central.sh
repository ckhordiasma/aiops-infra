#!/usr/bin/env bash
# add-component-to-rhoai-konflux-central.sh — Adds Tekton PipelineRun YAML to rhoai-konflux-central
#
# Usage:
...
BRANCH_NAME="add-${COMPONENT_NAME}-pipeline"
echo "Creating branch: $BRANCH_NAME from upstream/$TARGET_BRANCH"

git fetch upstream
git checkout -b "$BRANCH_NAME" "upstream/$TARGET_BRANCH"

# ── Add PipelineRun YAML ──────────────────────────────────────────────────────

echo "Creating pipelines directory if needed..."
mkdir -p pipelines

# Generate <component-name>.yaml
cat > "pipelines/$COMPONENT_NAME.yaml" <<EOF
...
yamllint "pipelines/$COMPONENT_NAME.yaml"

# ── Commit and push ───────────────────────────────────────────────────────────

bash "$SCRIPTS_DIR/git_commit_push.sh" \
  --clone-dir "$WORKDIR/rhoai-konflux-central" \
  --files "pipelines/$COMPONENT_NAME.yaml" \
  --message "Add Konflux pipeline for $COMPONENT_NAME" \
  --branch "$BRANCH_NAME" \
  --remote origin
...
PR_URL=$(uv run --script "$SCRIPTS_DIR/raise_github_pr.py" \
  ...
  --title "Add Konflux pipeline for $COMPONENT_NAME" \
  --description "Adds Tekton PipelineRun configuration for $COMPONENT_NAME targeting RHOAI $TARGET_VERSION.
...
if [[ "$JIRA_ENABLED" == "true" ]]; then
  ...
  uv run --script "$SCRIPTS_DIR/update_jira_issue.py" \
    --jira-url "$JIRA_URL" \
    --add-labels "rkc-pr-raised" \
    --comment "RHOAI Konflux Central PR raised: $PR_URL"
else
